...
/**
 * A scale a value can also be written back at, and the only place either direction is crossed.
 *
 * <p>{@link OrderScale} reads a rule; this writes a value. A type takes part in the reading by
 * having an order its literals sit on, and takes part in this by having an order-preserving count
 * <em>and a way back</em>. Whether a type has the way back is this question, asked here and nowhere
 * else: it was answered in three places that disagreed — a predicate deciding what a report said, a
 * reader deciding what an invariant bounded, and a table deciding how a value was written back — so
 * a {@code Date} was a carrier to one of them and not to another, and the disagreement showed up as
 * a bound that vanished without a word.
 *
 * <p><b>Closed both ways.</b> Everything that turns a value into a {@link Count} and everything that
 * turns a {@link Count} back into a value is a method here or on {@link OrderScale}. Outside them
 * nothing may read a count as a number a model wrote, or build a value out of one: those were the
 * leaks, and each of them was a reader that had a carrier available and did not ask it. A reader
 * that sniffed a written temporal for a {@code T} to decide whether it was counting days or seconds
 * is the shape they all had — the declared type says which, and guessing from the text is a second
 * answer to a question already answered.
 *
 * <p><b>Sealed, so a carrier added is one every reader has to answer for.</b> Each switch below is
 * over these cases and nothing else, which is what makes a seventh one a build failure rather than a
 * wrong value. That mattered least while every carrier's counts looked like numbers a model might
 * write; an ordinal is a small integer, which is the most plausible-looking wrong value of the six
 * and the one a report is least likely to give away.
 *
 * <p><b>Which types have one</b> is {@link #ofValue}, and it narrows {@link OrderScale#ofValue}
 * rather than deciding again. A carrier is a scale that also writes, so there is one table of which
 * types are ordered and one question asked of it — a second table here is what the readers above
 * disagreed through.
 */
public sealed interface Carrier extends OrderScale {
     ...
     * it.
     *
     * <p>{@link OrderScale#ofValue} narrowed, and not a second table. Which types are ordered is one
     * question with one answer; which of those a value can be written back at is this one, and it is
     * answered by asking whether the scale is a carrier. A `Time` and an `Instant` are ordered and
     * are not carriers: each has a count that would embed — a second of the day, a nanosecond from
     * an epoch — and neither has been given the way back that a line drawn at it needs, so a rule
     * written over one is read and no row is asked for at it (spec
     * §a-line-is-drawn-where-the-values-can-carry-one).
     */
    static Carrier ofValue(Type type, Symbols symbols) {
        return OrderScale.ofValue(type, symbols) instanceof Carrier carrier ? carrier : null;
    }
