...
public final class CoverageSites {
    ...
    private static final class Walk {
        ...
        private void comparisons(Core condition) {
            for (Core.Binary comparison : comparisonsOf(condition)) {
                // Numbered once. A node reached twice is one comparison written once, and a second
                // number for it would be a site the emitter never lights — which is the shape of a
                // real omission and would be reported as one.
                if (byComparison.containsKey(comparison)) {
                    continue;
                }
                // Keyed on where the comparison was written and not on the fork testing it. A
                // condition can be an application of a function parameter, and then the comparison
                // is the caller's: two predicates written separately are two lines, and one
                // predicate handed to two calls is one, neither of which the fork can say.
