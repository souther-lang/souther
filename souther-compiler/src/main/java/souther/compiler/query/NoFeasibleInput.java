 ...
 * only place it is.
 *
 * <p><b>One fact about the behavior, and every measure of its input reads it.</b> A partition and a
 * border are both about values of that input, so both are inapplicable for this one reason and
 * neither works it out. Asked per measure, two of them would be free to disagree about one model;
 * asked per rule or per position, it would be said once for each and every time about a rule that is
 * not at fault — two clauses each admitting values are empty together, and neither of them is the
 * one that failed.
 *
