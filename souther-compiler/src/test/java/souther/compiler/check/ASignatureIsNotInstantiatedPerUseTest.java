...
class ASignatureIsNotInstantiatedPerUseTest {
    ...
    void twoLibrarySignaturesSpellTheirVariableTheSame() {
        Type fromList = souther.compiler.DefaultStdlib.get().entry("List.length").signature().params().get(0);
        Type fromSet = souther.compiler.DefaultStdlib.get().entry("Set.size").signature().params().get(0);
    ...
    void aVariableTheExpectedTypeSettledComesBackAsThatType() {
        Stdlib.Signature get = souther.compiler.DefaultStdlib.get().entry("List.get").signature();
