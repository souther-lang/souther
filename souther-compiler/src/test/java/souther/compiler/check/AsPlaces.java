...
final class AsPlaces {
    ...
    static FactSubject of(BindingId binding) {
        return new Terms(Symbols.none(), souther.compiler.check.ReadAs.THE_COMPILATION_DOES).placeSubject(binding);
    }

    static Term term(BindingId binding) {
        return new Terms(Symbols.none(), souther.compiler.check.ReadAs.THE_COMPILATION_DOES).placeTerm(binding);
    }
}
