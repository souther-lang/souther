 ...
 * a map" ({@code fold} + {@code Map.updateOrInsert}). The surrounding declared type (a record field, a helper
 * return) pins the accumulator so the empty seed no longer forces its value type to a bottom.
 */
class CompileMapUpsertFoldTest {

    // REPRO 1 (primary): fold + Map.updateOrInsert with a Map.empty seed, in a record-field context.
    @Test
    void countByLabelWithUpsertAndEmptySeed() throws Exception {
        String src = """
                ...
                let run (i) = Out { m = fold((acc, k) -> Map.updateOrInsert(k, 1, n -> n + 1, acc), Map.empty, i.keys) }
                """;
    ...
    void countByIntKeyWithUpsertAndEmptySeed() throws Exception {
        String src = """
                ...
                let counted (ns: List<Int>) : Map<Int, Int> =
                    fold((acc, n) -> Map.updateOrInsert(n, 1, c -> c + 1, acc), Map.empty, ns)
    ...
    void foldWithEmptySeedUnderAnIfBranchIsPinnedByField() throws Exception {
        String src = """
                ...
                let run (i) = Out {
                    m = if i.on
                        then fold((acc, k) -> Map.updateOrInsert(k, 1, n -> n + 1, acc), Map.empty, i.keys)
                        else Map.empty
                }
                """;
        BytesClassLoader loader = new BytesClassLoader(Compiler.compile(src), getClass().getClassLoader());
    ...
    void recursiveHelperReturningAMapFoldsOverAnEmptySeedInTailPosition() throws Exception {
        String src = """
                ...
                partial let build (keys: List<String>, n: Int): Map<String, Int> =
                    if n <= 0 then fold((acc, k) -> Map.updateOrInsert(k, 1, x -> x + 1, acc), Map.empty, keys)
                    else build(keys, n - 1)
                behavior run : (i: In) -> Out constructs Out
