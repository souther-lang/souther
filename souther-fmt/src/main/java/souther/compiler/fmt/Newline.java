 ...
 *
 * <p>{@code indents} is false on the one break that writes no indent: the one leaving a blank line,
 * whose line has nothing on it. Its zero is not a column any nesting is written at, and a reader
 * taking it for one would have the level it stands under written at two columns.
 *
 ...
 */
record Newline(int offset, int indent, List<Doc.NestRef> under, Cause cause, boolean indents) {
