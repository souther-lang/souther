 ...
 *  shares structure with its input in O(log n) and never mutates it. Iteration order is
 *  implementation-defined and not first-seen order; two sets that are equal are not guaranteed to
 *  iterate in the same order, so neither a caller nor {@code toList} / encode may depend on a
 *  particular one ({@link PersistentHashSet}). This class is Raoh-free; {@link #fromList}
