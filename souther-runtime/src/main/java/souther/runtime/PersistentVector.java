...
public final class PersistentVector<E> extends AbstractList<E>
     ...
     *
     * <p>It is a {@link java.util.List} of what it has been given so far, because {@link Lists#build}
     * carries one as the accumulator of a fold that only grows its list: the accumulator is a
     * {@code List} everywhere the generated bytecode touches it, down to the {@code checkcast} on the
     * step's parameter. Reads answer from the half-built trie, so the value the walk carries is the
     * list it will become; what a builder does not offer is the mutators {@link AbstractList} refuses
     * and the sharing an immutable append gives, which is why nothing but a linear walk may hold one.
     */
    static final class Builder<E> extends AbstractList<E> {
        private int cnt = 0;
        ...
        /** Element {@code i} of what has been added so far: the tail holds the last {@code tailLen}
         *  of them and the trie the blocks already spilled into it. */
        @Override
        @SuppressWarnings("unchecked")
        public E get(int i) {
            if (i < 0 || i >= cnt) {
