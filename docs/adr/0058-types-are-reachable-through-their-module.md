...
- A behavior of another module is named through it too, as a `>->` stage and as a `requires`. A qualified behavior reference is rewritten to the bare name plus the import that brings it in, since a behavior's name is a member name in the generated class and the bare form is what the rest of the compiler needs — the qualifier only says which module to take it from, which is what an import says.
- One module cannot reach two behaviors of the same name. An injected behavior is a field named after it, so qualifying does not separate them; naming both is reported instead of one silently winning, which is what used to happen (the failure then surfaced as a type mismatch between two types both printed under one name). Whether a colliding pair should get mangled field names is left until an example asks for it: today's answer keeps the Java surface independent of who imported what, which is the property ADR-0057 refused to give up.
