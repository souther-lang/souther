...
- The double accessor `.value.value` disappears from the one place it occurred (the `addressValue` helper in the member example), and the example becomes the first non-primitive newtype in the corpus.
