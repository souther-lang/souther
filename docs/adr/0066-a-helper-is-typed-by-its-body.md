...
- A function-typed parameter MUST be written. Applying it does not determine its type, and the inliner needs the value/function distinction to expand the call. A `let`-bound lambda is not the same case: it is applied in the body that binds it, so its parameter types are read from those applications — the binding is expanded at each of them, and there is no declaration to tell function from value before that.
