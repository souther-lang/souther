...
- **Example evaluation is bounded by a counted budget.** The classes generated for an evaluation count what they go through: every loop the emitter emits counts its backward branch, and every recursive helper counts the frame it adds. A row that passes more counted points than the policy allows, or recurses deeper than it allows, is reported (`E1910`). The counting is in the code, so the code stops itself: a `partial` recursion that does not terminate ends its own evaluation rather than being asked to stop and going on. What the budget establishes is that the evaluation did not finish within it, which is weaker than non-termination and is what `E1910` says — a finite counter cannot tell a recursion that never stops from one that would have stopped shortly after. This is a safety net for `partial` code, not a proof.
...
What a dependency carries as source is counted; what stays in its jar is not. A published module travels as the source it was written as, so its types, its invariants and the `let`s it exposes are regenerated by the compilation that imports it and counted like anything else that compilation generates — a row that loops inside one of those spends the budget in there and is reported for it. A behavior's body is not carried: it stays in the jar it was built into, so a row that reaches one runs bytecode this compile did not generate, with no counted point in it, and what bounds it is the wait rather than the budget.

Closing that means publishing something an evaluation can re-instrument, which is a change to what an artifact carries and not something a budget can reach.

Where a dependency cannot be re-derived at all, the path's modules are left to the loader together rather than refused, and none of them is regenerated. A class defined by the evaluation's loader and one defined by the parent are different types under one binary name, so regenerating part of a dependency closure and leaving the rest would put two versions of a shared type into one evaluation — and what comes of that is a cast that cannot succeed, said about a model that is fine. All of them or none is what keeps an evaluation in one type world.
...
- `E1910` says the evaluation exceeded its budget, and not that the model does not terminate. A finite counter cannot establish the second, and reporting it as though it could told the author of a model that does terminate that it does not.
