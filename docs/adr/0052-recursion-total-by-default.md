...
A module read from the path is counted too, wherever this compiler can re-derive it: a published module travels as the source it was written as (<<published-modules>>), so asking for its evaluation classes asks for the same generation a source module gets. A row that steps into a dependency and loops there spends the budget in there and is reported for it. What is left uncounted is a dependency this compiler cannot re-derive at all, and there the module is left to the loader rather than refused — the name resolves from the jar, only the counting is missing, and a row that does not come back inside one is an evaluation that did not answer.
