...
That last is a fact about types, so the body is read as its reader types it, and what it names is what the emitter's own questions answer for it (`Ordering`, `Comparison`). A body whose parameters take their types from the call is read as written, and the emitter's refusal covers what that reading cannot see.
