...
that one generated component owns what a clause means, not that it has one Java method. It may
expose entry points specific to the evidence a caller has, and an answer known only as the case it
is has one of its own.
...
out there and nowhere else. It may expose entry points specific to the evidence a caller has, and a
caller chooses among them only by what it holds — never by knowing which rules follow from it.
Evidence with no entry point of its own is not held here at all: what states only one side of the
relation is held where the behavior answers.
...
the case an answer is, which is all an `example` row states where the case name it wrote does not
determine a value. It runs the
...
pair, and the declaration decides it. An arm is part of what a row wrote, and what a bare case name
states is decided by whether the name determines a value: a unit case has one value, so naming it
is writing the whole answer and it is held as one; where the name determines none, the row states
the case and no value, and is held to what that case decides on its own. A `_` row of a fake and a
