...
- A helper is held as it is closed over its own module, so a value or helper of that module it names is part of its copy and is not a target of its own: an edit to one of those moves the helper, and nothing kept by the module is named in another module's requirements. A block and an invariant are closed the same way.
- What a declaration is held as rests on what its module wrote and on nothing another module declares. A definition of another module a copy holds — a helper closing expanded into it, a value an invariant was settled with — is written into it as what names it, and is held on its own, as that definition: a module whose classes copy the declaration copies it too, and whatever it holds in turn. Written in instead, a declaration would be compared under one rule and the definition inside it under another — a constant written another way is one constant, and the invariant it was written into would be another invariant — and a module off the path, worked out again against what this compilation has, would offer something other than what its classes record.
...
- A body is held as a structure over what it means rather than as its text or a hash of it. Where the source put a term, how it spelled a name that resolves to the same declaration, and the names the compiler gave its bindings are left out; a binding is written as the place it is bound at, and the departures of an attempted construction by the clause each answers. What the checker worked out is not held: the type of a helper's parameter its author left unwritten, and the types an expansion carries, which are its callee's signature instantiated at the call. Each follows from what is held and from the rules `BOUNDARY_VERSION` stands for, and a parameter says which of its types its author wrote. Its text would refuse a module for a comment, and a hash would make a promise of the artifact rest on no two bodies colliding.
