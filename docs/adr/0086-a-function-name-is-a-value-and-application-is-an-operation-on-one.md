...
- Every standard-library function and value states its own signature in a core declaration. `Prelude.BUILTINS` is empty. What the type language cannot say — an ordering constraint, "this argument is a rounding mode" — is a side condition beside a declared signature, never in place of one.
