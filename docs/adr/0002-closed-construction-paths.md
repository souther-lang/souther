...
`constructs` is **not** what guards the invariant — `__construct` and the decoder do that, and both check on every construction path (see ADR-0003). Deleting `constructs` would not let anyone build an unvalidated `VerifiedEmail`. What `constructs` buys is *model reading*: from the declaration alone you can tell whether a behavior **mints** a new value or merely **passes through** one it was given, which the output type cannot tell you. But that is a reading aid, not a dependency. Unlike `depends on`, a construction permission is invisible to callers — whether a behavior mints Member or passes it through, the caller calls it identically — so inferring it changes no outward contract (the failure mode that makes `depends on` worth declaring, ADR-0017, does not arise here). So on a let-backed behavior, whose body is visible, `constructs` is **optional** — the optional-inferred rule and its exact checks belong to ADR-0017 and are not restated here. It lives on `behavior` because "this behavior produces a new PreApproved" is a statement of the spec (exactly what the DSL's state transition says) for whoever chooses to write it.
