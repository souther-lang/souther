...
**A published value's names are resolved where it was written** (ADR-0067), and what crosses is *closed* over names and not over bodies: every helper in it is expanded, and every value it names stays a reference under the name of the module that declares it, carried along with it. A reader that spells one of those names the same way reaches its own definition and not the published one, so `pricing.standard = Amount(base)` read in a module with its own `base` still answers the published number. The values a published one names are not offered to the importer: they can be written by nobody but the definitions that carry them. A value that only a published definition names has an entry too, since the reader that expands the definition reaches it; sharing such a value between exports is given up, in exchange for a type the module keeps to itself never being named from outside it.
