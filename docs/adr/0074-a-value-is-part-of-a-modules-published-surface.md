...
The entry is a definition of the declaring module, made as such (`DefinitionRole.PublishedValueEntry`) and neither a row's value nor a `let` the module wrote: its body is a reference to the value and nothing else, so the region that reads the reference decides what the value needs and builds each of those once. No second account of a value's dependencies is kept for it.

**A constant has no execution to place.** A value that folds to a number, a string or a boolean is known when the module reading it is compiled, and a reference to it is that literal wherever it is written: in the module that declares it, in another module of the same run and in one reading it from a jar. That is ADR-0072's rule for a constant and it is what lets a pattern be composed of a published value, which has to be known before anything runs. So the entry is where a value that is computed runs, and a constant is not called from anywhere; the module still publishes an entry for it, so that the surface a module offers to Java does not depend on whether a value happens to fold. What a reader does with a value follows from what the value is and not from which module declared it or how the reader came by it, and a test holds that for both readings.
